...
The kernel sees `#!/bin/bash\r` and looks for a program at the path `/bin/bash\r`, which doesn't exist. The `\r` is invisible in most text editors but it's there.

This is compounded by Git's `autocrlf=true` setting on Windows, which converts LF to CRLF on checkout even if the file was committed correctly.
