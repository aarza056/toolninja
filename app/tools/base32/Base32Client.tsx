...
import { base32EncodeText, base32DecodeToText, base32Encode, base32Decode } from "@/lib/base32";
import { bytesToHex, hexToBytes } from "@/lib/base58";

const STORAGE_KEY = "toolninja:base32";
...
export default function Base32Client() {
  ...
  const { output, error } = useMemo(() => {
    ...
    try {
      if (mode === "encode") {
        const result = format === "text" ? base32EncodeText(input) : base32Encode(hexToBytes(input));
        return { output: result, error: "" };
      }
      const result = format === "text" ? base32DecodeToText(input.trim()) : bytesToHex(base32Decode(input.trim()));
      return { output: result, error: "" };
    } catch (e) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[calc(100vh-320px)] min-h-[350px]">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-[#888888] font-medium">
            {mode === "encode" ? (format === "text" ? "Text" : "Hex bytes") : "Base32 string"}
          </label>
          <textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder={mode === "encode" ? (format === "text" ? "Hello World!" : "48656c6c6f") : "JBSWY3DPEBLW64TMMQ======"}
            spellCheck={false}
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between">
            <label className="text-xs text-[#888888] font-medium">
              {mode === "encode" ? "Base32 string" : format === "text" ? "Text" : "Hex bytes"}
            </label>
